...
public class PlayingFieldControllerOld : MonoBehaviour
{
    public static PlayingFieldControllerOld Instance;
    ...
    private void FillTheField()
    {
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                PlayingField.MatrixOfTiles[x, y].TilePrefab.GetComponent<TileControllerOld>().TileIndex =
                    new Vector2Int(x, y);
            }
        }
    }
    ...
    private TypeOfTile GetValidRandomTypeOfTile(int x, int y)
    {
        ...
        if (x >= 2)
        {
            while (true)
            {
                if (!IsAnyRowStacked(randomType, x, y))
                    break;
                else
                {
        ...
        if (y >= 2)
        {
            while (true)
            {
                if (!IsAnyColumnStacked(randomType, x, y) && excludedType != randomType)
                    break;
                else
                {
                    randomType = GetRandomTypeOfTile();
    ...
    private bool IsAnyRowStacked(TypeOfTile typeOfTile, int x, int y)
    {
        return (typeOfTile == PlayingField.MatrixOfTiles[x - 1, y].TypeOfTile) &&
               (typeOfTile == PlayingField.MatrixOfTiles[x - 2, y].TypeOfTile);
    }

    private bool IsAnyColumnStacked(TypeOfTile typeOfTile, int x, int y)
    {
